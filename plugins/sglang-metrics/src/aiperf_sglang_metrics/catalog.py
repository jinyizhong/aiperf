...
Defaults follow SGLang production_metrics. Unknown SGLang families are kept in
Other. Custom transfer instrumentation can be supplied through settings.metrics.
"""
...
# name: title, group (None = endpoint role), unit, scale, interpretation
CATALOG = {
    ...
    "sglang:token_usage": ("Token 池使用率", "kvcache", "%", 100, "KV/Token 池口径，不是整卡 HBM 使用率"),
    ...
    "sglang:num_retracted_reqs": ("回退请求", "kvcache", "requests", 1, "按来源的 Prometheus TYPE 解释"),
    ...
    "sglang:kv_transfer_speed_gb_s": ("KV 传输速度", "transfer", "GB/s", 1, "引擎上报的传输速度，不是全部 NIC 带宽"),
    "sglang:num_prefill_bootstrap_queue_reqs": ("等待 Bootstrap", "transfer", "requests", 1, "可选指标，取决于 SGLang 版本"),
    "sglang:num_decode_transfer_queue_reqs": ("传输中请求", "transfer", "requests", 1, "可选指标，取决于 SGLang 版本"),
    "sglang:num_decode_prealloc_queue_reqs": ("等待预分配", "transfer", "requests", 1, "可选指标，取决于 SGLang 版本"),
}
...
def describe(name: str, role: str, override: dict[str, Any]) -> dict[str, Any]:
    ...
    base = CATALOG.get(name.removesuffix("_total"),
                       (name, "other", "", 1, "原始指标；请结合 exporter HELP 确认口径"))
